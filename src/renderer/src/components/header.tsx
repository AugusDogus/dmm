...
export function Header() {
    ...
    trpc.watchPatchStatus.useSubscription(undefined, {
        onData: (isPatched) => {
            setIsPatched(isPatched as boolean);
        }
    });
    ...
    return <header className='flex items-center px-4 pt-4 w-full max-w-7xl'>
        <div className='flex flex-row gap-x-4'>
            ...
            <div className='inline-flex justify-center items-center py-2 h-10 text-sm font-medium whitespace-nowrap rounded-md transition-colors ring-offset-background focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 disabled:pointer-events-none disabled:opacity-50'>
                {isPatched ?
                    <p className='flex flex-row gap-2 items-center'><BadgeCheck className='w-4 h-4 text-green-500' /> Patched</p> :
